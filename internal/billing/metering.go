...
type Meter struct {
	...
	// goroutine so Stop can wait for it to exit.
	//
	// Lifecycle state (started/stopped) is guarded by mu. Tier 101
	// replaced the previous startOnce/stopOnce pair because concurrent
	...
	// state machine, once Stop has set stopped=true, Start becomes a
	// no-op before it can touch wg at all.
...
// Start is also a no-op if Stop has already run, so a concurrent
// Stop-wins race cannot leave an Add-after-Wait reuse bug on wg.
func (m *Meter) Start() {
	m.mu.Lock()
	defer m.mu.Unlock()
