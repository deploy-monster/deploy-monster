...
func TestAutoRollbackManager_Start(t *testing.T) {
	...
	// Drain the async subscription AND the manager's own dispatch wg so
	// the handleFailure write has happened-before the test read below.
	// Pre-Tier-101 this used time.Sleep which let the race detector catch
	// mockStore.appStatusUpdates being appended concurrently with the test
	// reading it.
	bus.Drain()
	ar.Wait()
