...
// Always emits Connection: close because handleExecConnection serves
// exactly one request per TCP accept and then tears the socket down.
// Without this header, the Docker client's http.Transport would keep
// the idle connection in its pool and reuse it for the next request,
// racing our server-side Close and producing intermittent "use of
// closed network connection" failures (first observed in CI on the
// TestDockerManager_Exec_EmptyOutput run that broke Tier 102).
func writeHTTPResponse(conn net.Conn, status int, statusText string, headers map[string]string, body string) {
	...
	if _, hasConn := headers["Connection"]; !hasConn {
