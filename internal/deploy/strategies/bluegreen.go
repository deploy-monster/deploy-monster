...
//
//   - If the context is cancelled during the holdback, the strategy
...
func (b *BlueGreen) Execute(ctx context.Context, plan *DeployPlan) error {
	...
	// about to be) swapped by the route watcher. A cancelled ctx
	...
	if holdback > 0 {
		...
		case <-ctx.Done():
			if plan.Logger != nil {
				plan.Logger.Warn("blue-green: holdback cancelled, rolling back to blue",
					"green", greenID,
					"blue", plan.OldContainerID,
				)
			}
			b.cleanupGreen(context.Background(), plan, greenID)
			return fmt.Errorf("blue-green: promotion cancelled during holdback: %w", ctx.Err())
		}
	}
...
// Runs under a fresh context when the outer ctx has been cancelled
