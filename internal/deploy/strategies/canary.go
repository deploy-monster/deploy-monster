...
//   - If the context is cancelled between phases, remove the new
...
func (c *Canary) Execute(ctx context.Context, plan *DeployPlan) error {
	...
	for i, phase := range phases {
		...
		if phase.Dwell > 0 {
			...
			case <-ctx.Done():
				if plan.Logger != nil {
					plan.Logger.Warn("canary: cancelled during dwell, rolling back",
						"phase", i+1, "percent", phase.Percent,
					)
				}
				c.rollback(context.Background(), plan, cfg, newID)
				return fmt.Errorf("canary: cancelled during phase %d (%d%%): %w",
