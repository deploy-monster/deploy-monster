...
func (m *mockStore) ListDeploymentsByStatus(_ context.Context, _ string) ([]core.Deployment, error) {
	return nil, nil
}
...
// TestRolling_Execute_HealthCheckChaosRecovery asserts that the rolling
// strategy tolerates transient health-check failures and eventually converges
// once the container reports healthy.
func TestRolling_Execute_HealthCheckChaosRecovery(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping rolling chaos test with sleep in short mode")
	}

	calls := 0
	chaosRuntime := &mockRuntime{}
	chaosRuntime.statsFn = func(_ context.Context, _ string) (*core.ContainerStats, error) {
		calls++
		if calls == 1 {
	...
	plan := &DeployPlan{
		...
		Deployment:     &core.Deployment{Version: 1},
		NewImage:       "nginx:latest",
		OldContainerID: "old-container-456",
	...
	if !chaosRuntime.createCalled {
		t.Error("CreateAndStart should be called")
	}
	if !chaosRuntime.stopCalled {
		t.Error("old container should be stopped after new one is healthy")
	}
	if calls < 3 {
		t.Errorf("expected at least 3 health checks, got %d", calls)
	}
}

// TestRolling_Execute_ContainerDiesMidHealthCheck asserts that if the new
// container dies before becoming healthy, the deployment fails and the new
// container is cleaned up.
func TestRolling_Execute_ContainerDiesMidHealthCheck(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping rolling chaos test with sleep in short mode")
	}
	...
	chaosRuntime.statsFn = func(_ context.Context, _ string) (*core.ContainerStats, error) {
		calls++
		if calls < 2 {
			// Container is running but has no explicit health check yet
			return &core.ContainerStats{Health: "", Running: true}, nil
		}
		// Then it dies
		return &core.ContainerStats{Health: "unhealthy", Running: false}, nil
	}
	...
	err := rolling.Execute(context.Background(), plan)
	if err == nil {
		t.Fatal("expected error when container dies before becoming healthy")
	}
	if !chaosRuntime.stopCalled {
		t.Error("new container should be stopped on failure")
	}
	if !chaosRuntime.removeCalled {
		t.Error("new container should be removed on failure")
	}
	if calls < 2 {
