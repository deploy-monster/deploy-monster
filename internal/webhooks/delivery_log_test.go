...
func TestDeliveryTracker_RecordAndRetrieve(t *testing.T) {
	...
	// Drain async dispatch so the handler's bolt.Set has
	// happened-before the RecentFailures read below. Pre-Tier-101 this
	// test used time.Sleep which let -race catch the mockBoltStore
	// map write concurrent with the read.
	events.Drain()
