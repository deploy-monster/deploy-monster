...
// TestCanary_ZeroPercentNeverRoutesCanary is the "safety" baseline:
// percent=0 MUST mean no canary traffic, full stop, even if the canary
// pool is populated and healthy. Anything else is a canary-misroute
// incident.
func TestCanary_ZeroPercentNeverRoutesCanary(t *testing.T) {
	c := NewCanary([]string{"s1", "s2"}, []string{"c1"}, 0)
	for range 500 {
		got := c.Next(nil, &http.Request{})
		if got == "c1" {
			t.Fatalf("percent=0 routed to canary backend %q", got)
			return
		}
	}
}

// TestCanary_HundredPercentAlwaysRoutesCanary mirrors the other extreme —
// a fully promoted canary sends everything to the new pool, the stable
// pool is idle. The blue/green cutover relies on this.
func TestCanary_HundredPercentAlwaysRoutesCanary(t *testing.T) {
	c := NewCanary([]string{"s1"}, []string{"c1", "c2"}, 100)
	seenStable := false
	for range 500 {
		got := c.Next(nil, &http.Request{})
		if got == "s1" {
	...
	if seenStable {
		t.Error("percent=100 routed some traffic to stable pool, want zero")
	}
}

// TestCanary_5PercentSplit — the common "ramp up" starting point. Over
// 1000 picks with percent=5, canary pool must see ~50 picks. Smooth-WRR
// is exact over full cycles so we can assert the exact count modulo
// one incomplete cycle at the end.
func TestCanary_5PercentSplit(t *testing.T) {
	c := NewCanary([]string{"s1", "s2", "s3"}, []string{"c1"}, 5)
	canaryHits := 0
	total := 1000
	for range total {
		if c.Next(nil, &http.Request{}) == "c1" {
			canaryHits++
		}
	}
	// Expect 5% ± one incomplete cycle (≤1 off). Smooth-WRR is exact.
	want := 50
	if canaryHits < want-1 || canaryHits > want+1 {
		t.Errorf("canary hits = %d / %d, want %d ± 1", canaryHits, total, want)
	}
}

// TestCanary_SetPercentRampUp — confirms that increasing percent at
// runtime shifts the split proportionally. This is the live-ramp
// scenario an operator drives during a deploy.
func TestCanary_SetPercentRampUp(t *testing.T) {
	c := NewCanary([]string{"s1"}, []string{"c1"}, 10)

	count := func(percent int, n int) int {
	...
	if got := count(10, 100); got < 9 || got > 11 {
		t.Errorf("10%% canary over 100 picks: got %d hits, want 10±1", got)
	}
	if got := count(50, 100); got < 49 || got > 51 {
		t.Errorf("50%% canary over 100 picks: got %d hits, want 50±1", got)
	}
	if got := count(100, 100); got != 100 {
		t.Errorf("100%% canary over 100 picks: got %d hits, want 100", got)
	}
}

// TestCanary_EmptyCanaryPoolFallsBackToStable — constructing a Canary
// with percent>0 but empty canary pool must not 5xx. This protects
// against a config typo or a canary pool that hasn't rolled out yet.
func TestCanary_EmptyCanaryPoolFallsBackToStable(t *testing.T) {
	c := NewCanary([]string{"s1", "s2"}, nil, 25)
	for range 100 {
		got := c.Next(nil, &http.Request{})
		if got != "s1" && got != "s2" {
			t.Fatalf("empty canary pool returned non-stable backend %q", got)
		}
	}
}

// TestCanary_PercentClamped — NewCanary must clamp to [0,100].
func TestCanary_PercentClamped(t *testing.T) {
	tests := []struct {
		in, want int
	}{
		{-10, 0},
	...
	for _, tt := range tests {
		c := NewCanary([]string{"s"}, []string{"c"}, tt.in)
		if got := c.Percent(); got != tt.want {
			t.Errorf("NewCanary(percent=%d).Percent() = %d, want %d", tt.in, got, tt.want)
		}
	}
}

// TestCanary_PerPoolWeights — stable and canary pools each have their
// own Weighted so operators can express uneven capacity within a pool
// (e.g. two unequal-sized stable boxes). StableWeights/CanaryWeights
// expose those inner balancers for configuration.
func TestCanary_PerPoolWeights(t *testing.T) {
	c := NewCanary([]string{"s1", "s2"}, []string{"c1"}, 0)
	c.StableWeights().SetWeight("s1", 3)
	c.StableWeights().SetWeight("s2", 1)

	counts := map[string]int{}
	for range 400 {
		counts[c.Next(nil, &http.Request{})]++
	}
	// s1:s2 should be 3:1 across 400 picks = 300:100.
	if counts["s1"] != 300 || counts["s2"] != 100 {
		t.Errorf("per-pool weights: s1=%d s2=%d, want 300/100", counts["s1"], counts["s2"])
	}
}
