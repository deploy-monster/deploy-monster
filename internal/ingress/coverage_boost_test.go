...
func TestModule_httpHandler_ACMEChallenge(t *testing.T) {
	...
	// ACME challenges return 404 currently (placeholder)
	if rr.Code != http.StatusNotFound {
		t.Errorf("expected 404 for ACME challenge, got %d", rr.Code)
	}
}

func TestModule_httpHandler_ShortACMEPath(t *testing.T) {
	// Path that starts with /.well-known/acme-challenge/ prefix but is exactly that
	// length or shorter should be treated as a redirect, not ACME
	m := New()
	m.logger = slog.Default()
	m.certStore = NewCertStore()
	m.acme = NewACMEManager(m.certStore, "test@example.com", true, slog.Default())
	...
	// Path exactly /.well-known/acme-challenge/ (no token) should redirect
	...
	// This path has length == len("/.well-known/acme-challenge/"), so the condition
	// r.URL.Path[:len("/.well-known/acme-challenge/")] matches but len > len is false,
	// so it falls through to redirect
	if rr.Code != http.StatusMovedPermanently {
		t.Errorf("expected 301 redirect for bare ACME path, got %d", rr.Code)
	}
}
