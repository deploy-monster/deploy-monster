...
func TestFirstRunSetup_MissingEnvVars(t *testing.T) {
	store := &mockStore{
		userCount:      0,
	...
	// Clear env vars — Init should now fail because they are required
	...
	if err == nil {
		t.Fatal("expected error when admin env vars are missing")
	}
}
