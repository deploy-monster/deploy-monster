...
func TestModule_Send_ProviderReturnsError_EmitsFailureEvent(t *testing.T) {
	...
	// Drain async handlers so the subscriber write to failureReceived
	// happens-before the read below. Tier 101: pre-fix, -race caught
	// the async goroutine write racing with the main-goroutine read.
	events.Drain()

	if !failureReceived {
		t.Error("expected failure event to be received after drain")
	}
}
...
func TestModule_Send_ProviderSuccess_EmitsSuccessEvent(t *testing.T) {
	...
	// Drain async handlers so the subscriber's write to
	// successReceived happens-before the read below; pre-Tier-101 -race
	// caught the goroutine write racing with the main-goroutine read.
	events.Drain()

	if !successReceived {
		t.Error("expected success event to be received after drain")
	}
}
