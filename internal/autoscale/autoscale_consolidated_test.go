...
// === merged from autoscale_extra_test.go ===
...
func TestEvaluateAll_SkipsDisabledConfig(t *testing.T) {
	kv := newTestBolt()
	if err := kv.Set("autoscale", "app-1", autoscaleConfig{Enabled: false, MinReplicas: 1, MaxReplicas: 3, CPUTarget: 80}, 0); err != nil {
		t.Fatalf("seed config: %v", err)
	}
	kv.setErr = errors.New("should not be called") // Set should not be called for disabled config
...
func TestEvaluateAll_WithEnabledConfig(t *testing.T) {
	kv := newTestBolt()
	if err := kv.Set("autoscale", "app-1", autoscaleConfig{Enabled: true, MinReplicas: 1, MaxReplicas: 3, CPUTarget: 80, RAMTarget: 90}, 0); err != nil {
		t.Fatalf("seed config: %v", err)
	}
...
func TestEvaluateAll_GetConfigErrorLogsWarning(t *testing.T) {
	...
	kv.getErr = fmt.Errorf("decode failed: %w", core.ErrKVNotFound) // This triggers the inner err check
	// Actually, we want a non-KVNotFound error
	bolt2 := newTestBolt()
	bolt2.getErr = errors.New("kv db error")
