...
type SyncQueue struct {
	...
	// stopOnce guards close(stopCh) against double-Stop panics;
	// startOnce guards against a double-Start goroutine leak.
	...
	// mu serialises the closed flag with Enqueue so a Stop that races
	// a concurrent Enqueue cannot accept a job the worker will never
	// see.
	mu     sync.Mutex
	closed bool
}
...
// fights the first for every job and leak on Stop.
