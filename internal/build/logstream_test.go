...
func TestLogStore_EmitsBuildLogEvents(t *testing.T) {
	...
	// SubscribeAsync runs handlers in a semaphore-bounded goroutine,
	// so give it a moment to drain before asserting. A short sleep is
	// fine here — the async bus has no Drain/Wait API we can use.
