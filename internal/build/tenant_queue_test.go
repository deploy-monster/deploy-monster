...
func TestTenantQueue_ShutdownUnblocksPendingSubmits(t *testing.T) {
	...
	// Shutdown must unblock the pending Submit with
	// ErrTenantQueueClosed. Release the parker concurrently so
	// Shutdown's drain doesn't hang either.
	go close(occupyRelease)
	...
	case <-time.After(time.Second):
		t.Fatal("pending Submit never returned after Shutdown")
	}
}
