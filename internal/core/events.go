...
//
// Tier 101: the wrapper goroutine is now tracked by asyncWG so Drain
// waits for both the outer dispatch AND any inner per-handler goroutines
// Publish spawns. Pre-Tier-101, the outer goroutine was untracked: a
// test that called PublishAsync then Drain could observe Drain return
// before the outer goroutine had even entered Publish (and thus before
// it had Added the inner handler goroutine to asyncWG). Integration
// tests that relied on Drain ordering would then race with the leaked
// handler write.
