...
func (m *mockStore) UpdateTOTPEnabled(_ context.Context, _ string, _ bool, _ string) error { return nil }
...
func TestS3Storage_Upload_RequestCreationError(t *testing.T) {
	...
	err := s.Upload(context.TODO(), "key", strings.NewReader("d"), 1)
	if err == nil {
...
func TestS3Storage_Download_RequestCreationError(t *testing.T) {
	...
	_, err := s.Download(context.TODO(), "key")
	if err == nil {
		t.Error("expected error with nil context")
	}
}
