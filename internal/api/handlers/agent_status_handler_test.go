...
func TestAgentStatus_GetAgent_RemoteReturns404(t *testing.T) {
	// Phase 7.11: GetAgent used to echo any requested ID back as a fake
	// AgentNodeStatus, leaking existence of arbitrary server IDs to any
	// authenticated caller. It now returns 404 for every non-"local" ID
	// until a real remote-agent registry lookup lands.
	handler := NewAgentStatusHandler(testCoreForAgent())
