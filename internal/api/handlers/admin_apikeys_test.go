...
func TestCleanupExpiredKeys_RemovesExpired(t *testing.T) {
	...
	kv.Set("api_keys", "exp_abc", expiredRec, 0)
	kv.Set("api_keys", "val_xyz", validRec, 0)
	kv.Set("api_keys", "_index", apiKeyIndex{Prefixes: []string{"exp_abc", "val_xyz"}}, 0)
	...
	var idx apiKeyIndex
	if err := kv.Get("api_keys", "_index", &idx); err != nil {
	...
	if err := kv.Get("api_keys", "exp_abc", &check); err == nil {
...
func TestCleanupExpiredKeys_NoExpiry(t *testing.T) {
	...
	kv.Set("api_keys", "perm_abc", rec, 0)
	kv.Set("api_keys", "_index", apiKeyIndex{Prefixes: []string{"perm_abc"}}, 0)
...
func TestCleanupExpiredKeys_AllExpired(t *testing.T) {
	...
	kv.Set("api_keys", "exp1", apiKeyRecord{Prefix: "exp1", Hash: "h1", ExpiresAt: &past1}, 0)
	kv.Set("api_keys", "exp2", apiKeyRecord{Prefix: "exp2", Hash: "h2", ExpiresAt: &past2}, 0)
	kv.Set("api_keys", "_index", apiKeyIndex{Prefixes: []string{"exp1", "exp2"}}, 0)
