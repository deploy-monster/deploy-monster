...
func TestExecHandler_NoContainer(t *testing.T) {
	h := NewExecHandler(&mockContainerRuntime{containers: []core.ContainerInfo{}})
...
func TestTransferHandler_NotSuperAdmin(t *testing.T) {
	h := NewTransferHandler(newMockStore(), core.NewEventBus(nil))

	req := httptest.NewRequest("POST", "/api/v1/apps/app-1/transfer", strings.NewReader(`{}`))
	req.SetPathValue("id", "app-1")
	req = withClaims(req, "u1", "t1", "role_admin", "u@e.com")
	rr := httptest.NewRecorder()
...
func TestTransferHandler_NoClaims(t *testing.T) {
	h := NewTransferHandler(newMockStore(), core.NewEventBus(nil))
...
func TestTenantRateLimitHandler_Get_NoClaims(t *testing.T) {
	h := NewTenantRateLimitHandler(newMockBoltStore())
...
func TestTenantRateLimitHandler_Get_NotSuperAdmin(t *testing.T) {
	h := NewTenantRateLimitHandler(newMockBoltStore())
...
func TestTenantRateLimitHandler_Update_NoClaims(t *testing.T) {
	h := NewTenantRateLimitHandler(newMockBoltStore())

	req := httptest.NewRequest("PUT", "/api/v1/admin/tenants/t1/ratelimit", strings.NewReader("{}"))
	req.SetPathValue("id", "t1")
	rr := httptest.NewRecorder()
	h.Update(rr, req)
