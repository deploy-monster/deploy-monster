...
func (m *mockStore) ListDeploymentsByStatus(_ context.Context, status string) ([]core.Deployment, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	var out []core.Deployment
	for _, deps := range m.deploymentsByApp {
		for _, d := range deps {
			if d.Status == status {
				out = append(out, d)
			}
...
func (m *mockStore) UpdateDeployment(_ context.Context, d *core.Deployment) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	if d == nil {
		return nil
	}
	for appID, deps := range m.deploymentsByApp {
		for i := range deps {
			if deps[i].ID == d.ID {
				m.deploymentsByApp[appID][i] = *d
				return nil
			}
		}
	}
...
func (m *mockStore) AtomicNextDeployVersion(ctx context.Context, appID string) (int, error) {
	return m.GetNextDeployVersion(ctx, appID)
}
