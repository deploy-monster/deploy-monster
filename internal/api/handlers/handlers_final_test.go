...
// =============================================================================
// DBBackupHandler.Status — non-super-admin (line 65-66)
// =============================================================================

func TestFinal_DBBackup_Status_Forbidden(t *testing.T) {
	c := &core.Core{Config: &core.Config{}}
	h := NewDBBackupHandler(c)

	req := httptest.NewRequest("GET", "/api/v1/admin/db/status", nil)
	req = withClaims(req, "u1", "t1", "role_member", "user@test.com")
	rr := httptest.NewRecorder()
	h.Status(rr, req)
