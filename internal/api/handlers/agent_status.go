...
func (h *AgentStatusHandler) GetAgent(w http.ResponseWriter, r *http.Request) {
	...
	// No remote agent registry is wired up yet — echoing the caller's
	// ID back with fake fields leaked resource existence to anyone who
	// could guess an ID (see Phase 7.11 cross-tenant fuzz). Until the
	// node registry lookup lands, treat every non-"local" ID as 404.
