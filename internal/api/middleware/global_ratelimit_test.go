...
func TestGlobalRateLimiter_AllowlistBypassesSPARoutes(t *testing.T) {
	rl := NewGlobalRateLimiter(2, time.Minute)
	defer rl.Stop()
	rl.SetRateLimitedPrefixes([]string{"/api/", "/hooks/"})
	...
	// Blast the SPA routes well above the 2-per-window budget.
	// None of them should ever see a 429 because the allowlist
	// only matches /api/ and /hooks/.
	for i := 0; i < 20; i++ {
		for _, path := range []string{"/", "/login", "/register", "/assets/index.js", "/apps/123"} {
			if code := hit(path); code != http.StatusOK {
				t.Fatalf("iteration %d path %q: status = %d, want 200 (SPA must bypass)", i, path, code)
			}
		}
	}

	// /api/ still gets rate-limited: 2 allowed, 3rd blocked.
	if code := hit("/api/v1/apps"); code != http.StatusOK {
		t.Fatalf("first /api/ request: status = %d, want 200", code)
	}
	if code := hit("/api/v1/apps"); code != http.StatusOK {
		t.Fatalf("second /api/ request: status = %d, want 200", code)
	}
	if code := hit("/api/v1/apps"); code != http.StatusTooManyRequests {
		t.Errorf("third /api/ request: status = %d, want 429", code)
	}

	// /hooks/ shares the same per-IP budget as /api/ because we
	// already exhausted it with the /api/ burst above.
	if code := hit("/hooks/v1/deadbeef"); code != http.StatusTooManyRequests {
		t.Errorf("/hooks/ request after /api/ exhaustion: status = %d, want 429", code)
	}

	// But a fresh non-API path still bypasses cleanly.
	if code := hit("/login"); code != http.StatusOK {
		t.Errorf("/login after /api/ exhaustion: status = %d, want 200 (bypass)", code)
	}
}

func TestGlobalRateLimiter_EmptyAllowlistMatchesLegacyBehavior(t *testing.T) {
	rl := NewGlobalRateLimiter(1, time.Minute)
	defer rl.Stop()
	rl.SetRateLimitedPrefixes(nil) // explicit nil = rate-limit everything
	...
	handler.ServeHTTP(rr, req)
	if rr.Code != http.StatusTooManyRequests {
		t.Errorf("second /login request with nil allowlist: status = %d, want 429", rr.Code)
	}
}
