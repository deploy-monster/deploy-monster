...
func TestSecurityHeaders_SetsAllHeaders(t *testing.T) {
	...
	// HSTS must NOT be sent on plain HTTP (e.g. :8443 before certs).
...
// TestSecurityHeaders_CSPDirective locks in the exact CSP directive so any
// relaxation (e.g. re-adding data: fonts or ws:/wss: scheme sources) trips CI.
// Same-origin WebSocket traffic is covered by connect-src 'self' per CSP 3.
func TestSecurityHeaders_CSPDirective(t *testing.T) {
	handler := SecurityHeaders(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
	}))

	for _, path := range []string{"/", "/dashboard", "/api/v1/apps"} {
		req := httptest.NewRequest(http.MethodGet, path, nil)
		rec := httptest.NewRecorder()
		handler.ServeHTTP(rec, req)

		want := "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self'; connect-src 'self'; frame-ancestors 'none'; base-uri 'self'; form-action 'self'; object-src 'none'"
