...
## 11. Open Refactoring Items (from code-level audit)

> Merged from the companion `refactor.md` (2026-05-29 pass). All other items
> from that document have been resolved and verified.
...
- **Coverage-padding tests (P2-19):** 109+ files organized by uncovered-line, not behavior. Most are genuinely redundant with behavior tests; a systematic audit + merge/delete is needed.
...
Several handler paths (`dashboard.go`, `apps.go` list) issue per-app follow-up
queries (`GetLatestDeployment`, `ListDomainsByApp`) inside a loop. On SQLite
(MaxOpenConns=1), these serialize behind the single write connection,
amplifying latency. The batch methods (`GetLatestDeploymentsByAppIDs`,
`ListDomainsByAppIDs`) already exist but are not wired into all callers.

---

*Generated by Claude Code analysis of /home/ersinkoc/Codebox/deploy-monster*
*Last updated: 2026-05-16; open-items section merged 2026-07-29*
