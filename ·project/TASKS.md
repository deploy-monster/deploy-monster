...
- [ ] **T-1.1.1** — Initialize Go module: `go mod init github.com/deploy-monster/deploy-monster`
- [ ] **T-1.1.2** — Create full directory structure per SPECIFICATION.md §24 (all `internal/` subdirectories, `cmd/`, `web/`, `scripts/`, `docs/`, `marketplace/`)
- [ ] **T-1.1.3** — Create `Makefile` with targets: `build`, `dev`, `test`, `lint`, `clean`, `docker`
- [ ] **T-1.1.4** — Create `.gitignore` (bin/, web/dist/, web/node_modules/, *.db, *.db-wal, *.db-shm, coverage.out)
- [ ] **T-1.1.5** — Create `scripts/build.sh` — builds React UI then Go binary with ldflags (version, commit, date)
- [ ] **T-1.1.6** — Create `.goreleaser.yaml` for cross-platform release builds (linux/amd64, linux/arm64, darwin/amd64, darwin/arm64)
- [ ] **T-1.1.7** — Create `deployments/Dockerfile` — multi-stage build (node → go → scratch)
- [ ] **T-1.1.8** — Create `deployments/docker-compose.dev.yaml` — dev environment with mounted source

### 1.2 Core Engine

- [ ] **T-1.2.1** — Implement `internal/core/config.go`:
...
- [ ] **T-1.2.2** — Implement `internal/core/module.go`:
...
- [ ] **T-1.2.3** — Implement `internal/core/registry.go`:
...
- [ ] **T-1.2.4** — Implement `internal/core/events.go`:
...
- [ ] **T-1.2.5** — Implement `internal/core/errors.go`:
...
- [ ] **T-1.2.6** — Implement `internal/core/app.go`:
...
- [ ] **T-1.2.7** — Implement `internal/core/id.go`:
...
- [ ] **T-1.2.8** — Implement `cmd/deploymonster/main.go`:
...
- [ ] **T-1.3.1** — Implement `internal/db/module.go`:
...
- [ ] **T-1.3.2** — Implement `internal/db/sqlite.go`:
...
- [ ] **T-1.3.3** — Implement `internal/db/migrations/0001_init.sql`:
...
- [ ] **T-1.3.4** — Implement migration runner in `sqlite.go`:
...
- [ ] **T-1.3.5** — Implement `internal/db/bolt.go`:
...
- [ ] **T-1.3.6** — Implement `internal/db/models/` — Go structs for every table:
  ...
  - `audit_log.go`
- [ ] **T-1.3.7** — Implement repository layer — `internal/db/` query functions:
...
- [ ] **T-1.4.1** — Implement `internal/auth/module.go`:
...
- [ ] **T-1.4.2** — Implement `internal/auth/jwt.go`:
...
- [ ] **T-1.4.3** — Implement `internal/auth/password.go`:
...
- [ ] **T-1.4.4** — Implement `internal/auth/middleware.go`:
...
- [ ] **T-1.4.5** — Implement `internal/auth/rbac.go`:
...
- [ ] **T-1.4.6** — Implement `internal/auth/apikey.go`:
...
- [ ] **T-1.4.7** — Implement first-run setup:
...
- [ ] **T-1.5.1** — Implement `internal/api/module.go`:
...
- [ ] **T-1.5.2** — Implement `internal/api/router.go`:
...
- [ ] **T-1.5.3** — Implement `internal/api/helpers.go`:
...
- [ ] **T-1.5.4** — Implement `internal/api/handlers/auth.go`:
...
- [ ] **T-1.5.5** — Implement `internal/api/handlers/apps.go` (basic):
...
- [ ] **T-1.5.6** — Implement `internal/api/spa.go`:
...
- [ ] **T-1.5.7** — Implement `internal/api/middleware/ratelimit.go`:
...
- [ ] **T-1.5.8** — Implement health endpoint:
...
- [ ] **T-1.6.1** — Implement `internal/deploy/module.go`:
...
- [ ] **T-1.6.2** — Implement `internal/deploy/docker.go`:
...
- [ ] **T-1.6.3** — Implement `internal/deploy/deployer.go`:
...
- [ ] **T-1.6.4** — Implement Docker network management:
...
- [ ] **T-1.7.1** — Initialize React project:
...
- [ ] **T-1.7.2** — Setup Tailwind CSS 4.1:
...
- [ ] **T-1.7.4** — Implement authentication pages:
...
- [ ] **T-1.7.6** — Implement Applications list page:
...
- [ ] **T-1.7.7** — Implement App Detail page (basic):
...
- [ ] **T-1.7.8** — Implement API client layer:
...
- [ ] **T-1.8.1** — Unit tests for `internal/core/` — module registry resolve, event bus pub/sub, config loading
- [ ] **T-1.8.2** — Unit tests for `internal/auth/` — JWT generation/validation, password hash/verify, RBAC permission check
- [ ] **T-1.8.3** — Unit tests for `internal/db/` — migration runner, SQLite CRUD operations, BBolt set/get with TTL
- [ ] **T-1.8.4** — Integration test: full app bootstrap → login → list containers → stop/start
- [ ] **T-1.8.5** — Build test: `make build` produces single binary, `make docker` produces image
...
- [ ] **T-2.1.1** — Implement `internal/ingress/module.go`:
...
- [ ] **T-2.1.2** — Implement `internal/ingress/proxy.go`:
...
- [ ] **T-2.1.3** — Implement `internal/ingress/router.go`:
...
- [ ] **T-2.1.4** — Implement `internal/ingress/tls.go`:
...
- [ ] **T-2.2.1** — Implement `internal/ingress/acme.go`:
...
- [ ] **T-2.3.1** — Implement `internal/discovery/module.go`:
...
- [ ] **T-2.3.2** — Implement `internal/discovery/watcher.go`:
...
- [ ] **T-2.3.3** — Implement `internal/discovery/labels.go`:
...
- [ ] **T-2.3.4** — Implement `internal/discovery/health.go`:
...
- [ ] **T-2.4.1** — Implement `internal/ingress/lb/balancer.go`:
  - `Strategy` interface with `Next(backends, request) string`
  - Factory: `NewStrategy(name) Strategy`
- [ ] **T-2.4.2** — Implement `roundrobin.go` — atomic counter
- [ ] **T-2.4.3** — Implement `leastconn.go` — connection counting with mutex
- [ ] **T-2.4.4** — Implement `iphash.go` — FNV-32a hash of client IP
- [ ] **T-2.4.5** — Implement `weighted.go` — weighted round-robin for canary

### 2.5 Ingress Middleware

- [ ] **T-2.5.1** — Implement `internal/ingress/middleware/ratelimit.go` — per-route rate limiting
- [ ] **T-2.5.2** — Implement `internal/ingress/middleware/cors.go` — configurable CORS headers
- [ ] **T-2.5.3** — Implement `internal/ingress/middleware/compress.go` — gzip response compression
- [ ] **T-2.5.4** — Implement `internal/ingress/middleware/headers.go` — custom header add/remove

### 2.6 Domain Management

- [ ] **T-2.6.1** — Implement `internal/api/handlers/domains.go`:
...
- [ ] **T-2.6.2** — Implement auto-subdomain generation:
...
- [ ] **T-2.7.1** — Domain management page in React:
...
- [ ] **T-2.7.2** — App detail: add Domains tab showing linked domains + SSL status

### 2.8 Phase 2 Testing

- [ ] **T-2.8.1** — Unit tests: route matching (exact, wildcard, path prefix), label parsing
- [ ] **T-2.8.2** — Unit tests: LB strategies (round-robin distribution, IP hash consistency)
- [ ] **T-2.8.3** — Integration test: deploy labeled container → auto-route → HTTP request returns response
...
- [ ] **T-3.1.1** — Implement `internal/build/module.go` — Build module, depends on `deploy`
- [ ] **T-3.1.2** — Implement `internal/build/detector.go` — project type detection (14 types, file-based checks)
- [ ] **T-3.1.3** — Implement `internal/build/templates/` — embedded Dockerfile templates for: nodejs, nextjs, vite, nuxt, go, python, rust, php, java, dotnet, ruby, static
- [ ] **T-3.1.4** — Implement `internal/build/generator.go` — `GenerateDockerfile(projType, dir)` — select and render template
- [ ] **T-3.1.5** — Implement `internal/build/builder.go`:
...
- [ ] **T-3.1.6** — Implement `internal/build/git.go`:
...
- [ ] **T-3.1.7** — Implement build worker pool:
...
- [ ] **T-3.2.1** — Implement `internal/gitsources/module.go` — Git source module
- [ ] **T-3.2.2** — Implement `internal/gitsources/providers/provider.go` — GitProvider interface:
...
- [ ] **T-3.2.3** — Implement `internal/gitsources/providers/github.go` — GitHub API v3 integration
- [ ] **T-3.2.4** — Implement `internal/gitsources/providers/gitlab.go` — GitLab API v4 integration
- [ ] **T-3.2.5** — Implement `internal/gitsources/providers/gitea.go` — Gitea API integration
- [ ] **T-3.2.6** — Implement `internal/gitsources/providers/generic.go` — generic git clone (SSH/HTTPS, no API)
- [ ] **T-3.2.7** — Implement `internal/gitsources/oauth.go` — OAuth2 flow handler (GitHub, GitLab, Bitbucket callbacks)
- [ ] **T-3.2.8** — Implement Git Source API endpoints:
...
- [ ] **T-3.3.1** — Implement `internal/webhooks/module.go` — Webhook module
- [ ] **T-3.3.2** — Implement `internal/webhooks/receiver.go`:
...
- [ ] **T-3.3.3** — Implement `internal/webhooks/parsers/`:
...
- [ ] **T-3.3.4** — Implement webhook auto-registration:
...
- [ ] **T-3.3.5** — Implement webhook redeliver API: `POST /api/v1/webhooks/:id/redeliver/:log_id`

### 3.4 Deploy Strategies

- [ ] **T-3.4.1** — Implement `internal/deploy/strategies/recreate.go` — stop old → start new
- [ ] **T-3.4.2** — Implement `internal/deploy/strategies/rolling.go` — gradual replacement (start new → health check → stop old)
- [ ] **T-3.4.3** — Implement `internal/deploy/rollback.go`:
...
- [ ] **T-3.5.1** — Implement env var management API:
...
- [ ] **T-3.6.1** — Deploy Wizard (multi-step):
...
- [ ] **T-3.6.2** — Git Sources page: connected providers, add new (OAuth flow), repo browser
- [ ] **T-3.6.3** — Build log viewer: real-time WebSocket streaming, ANSI color support
- [ ] **T-3.6.4** — App detail: Deployments tab with version history, commit info, rollback button
- [ ] **T-3.6.5** — App detail: Environment tab with key-value editor + Monaco editor for bulk edit
- [ ] **T-3.6.6** — Webhooks page: list per app, delivery logs, redeliver button

### 3.7 WebSocket Endpoints

- [ ] **T-3.7.1** — Implement `internal/api/ws/logs.go` — real-time container log streaming
- [ ] **T-3.7.2** — Implement `internal/api/ws/exec.go` — container exec terminal (xterm.js ↔ WebSocket ↔ docker exec)
- [ ] **T-3.7.3** — Implement `internal/api/ws/events.go` — global event stream (deploy status, alerts)
- [ ] **T-3.7.4** — Implement build log streaming: WebSocket endpoint that streams build output in real-time
...
- [ ] **T-4.1** — Implement `internal/compose/parser.go` — parse Compose v2/v3 YAML into Go structs
- [ ] **T-4.2** — Implement `internal/compose/validator.go` — validate parsed compose (images exist, ports valid, no conflicts)
- [ ] **T-4.3** — Implement `internal/compose/interpolate.go` — `${VAR:-default}` variable interpolation, .env file support
- [ ] **T-4.4** — Implement `internal/compose/deployer.go` — ordered multi-service deploy (respect depends_on), network creation, volume creation, label injection
- [ ] **T-4.5** — Implement `internal/compose/converter.go` — compose service → DeployMonster Application model
- [ ] **T-4.6** — Implement Compose API: `POST/GET/PATCH/DELETE /api/v1/stacks`, `POST /stacks/:id/redeploy`, `POST /stacks/:id/services/:svc/scale`
- [ ] **T-4.7** — Implement Docker Image direct deploy: `POST /api/v1/apps` with `source_type=image` — pull any image, deploy with config
- [ ] **T-4.8** — UI: Compose Stacks page, stack detail with service list, per-service logs/restart/scale
- [ ] **T-4.9** — UI: Docker Image deploy flow in Deploy Wizard (enter image → configure → deploy)

---

## Phase 5 — Resource & Monitoring (v0.5.0)

- [ ] **T-5.1** — Implement `internal/resource/collector.go` — server metrics (CPU, RAM, disk, network) via /proc parsing
- [ ] **T-5.2** — Implement container metrics collection via Docker stats API
- [ ] **T-5.3** — Implement HTTP/ingress metrics (request count, latency percentiles, error rate, status codes)
- [ ] **T-5.4** — Implement metrics storage: 1-second ring buffer → 1-minute → 1-hour → 1-day rollups in SQLite
- [ ] **T-5.5** — Implement `internal/resource/alerts.go` — threshold-based alerts with duration, severity, notification dispatch
- [ ] **T-5.6** — Implement `internal/notifications/` — email (SMTP), Slack (webhook), Discord (webhook), Telegram (bot API)
- [ ] **T-5.7** — Implement metrics API: `GET /api/v1/servers/:id/metrics`, `GET /api/v1/apps/:id/metrics` with time range query
- [ ] **T-5.8** — UI: Server dashboard (CPU/RAM/disk gauges + history charts via Recharts)
- [ ] **T-5.9** — UI: App metrics tab (response time, request rate, error rate, resource usage vs limits)
- [ ] **T-5.10** — UI: Alert center in dashboard (active alerts, history, acknowledge)

---

## Phase 6 — Database & Backup (v0.6.0)

- [ ] **T-6.1** — Implement `internal/database/provisioner.go` — managed DB lifecycle (create volume → deploy container → health check → register)
- [ ] **T-6.2** — Implement `internal/database/engines/postgres.go` — PostgreSQL 14-17 provisioning with connection string generation
- [ ] **T-6.3** — Implement `internal/database/engines/mysql.go` — MySQL 8.x provisioning
- [ ] **T-6.4** — Implement `internal/database/engines/redis.go` — Redis 7.x provisioning
- [ ] **T-6.5** — Implement `internal/backup/volume.go` — Docker volume tar + gzip snapshot
- [ ] **T-6.6** — Implement `internal/backup/database.go` — pg_dump / mysqldump / redis BGSAVE wrappers
- [ ] **T-6.7** — Implement `internal/backup/storage/local.go` — local filesystem backup target
- [ ] **T-6.8** — Implement `internal/backup/storage/s3.go` — S3/S3-compatible upload (MinIO, R2, Backblaze)
- [ ] **T-6.9** — Implement `internal/backup/scheduler.go` — cron-based backup scheduling with retention policy
- [ ] **T-6.10** — Implement `internal/backup/encryption.go` — AES-256-GCM backup encryption
- [ ] **T-6.11** — Implement backup/database API endpoints
- [ ] **T-6.12** — UI: Database management page (create, connection string, backups, logs)
- [ ] **T-6.13** — UI: Backup management page (list, create, restore, storage targets config)

---

## Phase 7 — Secret Vault & Registration (v0.7.0)

- [ ] **T-7.1** — Implement `internal/secrets/vault.go` — AES-256-GCM encrypt/decrypt with Argon2id key derivation
- [ ] **T-7.2** — Implement `internal/secrets/store.go` — CRUD with versioning (last 10 versions per secret)
- [ ] **T-7.3** — Implement `internal/secrets/resolver.go` — `${SECRET:name}` resolution with scope chain
- [ ] **T-7.4** — Implement `internal/secrets/scoping.go` — global → tenant → project → app scope resolution
- [ ] **T-7.5** — Implement secret rotation: change value → auto-redeploy affected containers
- [ ] **T-7.6** — Implement .env import/export: `POST /api/v1/secrets/import`, `GET /api/v1/secrets/export`
- [ ] **T-7.7** — Implement secret masking in all log outputs (build logs, deploy logs, API responses)
- [ ] **T-7.8** — Implement registration modes: open, invite_only, approval, disabled, sso_only
- [ ] **T-7.9** — Implement invite system: create invite → email → accept with token → create user
- [ ] **T-7.10** — Implement approval queue: register → pending → admin approve/reject
- [ ] **T-7.11** — Implement SSO/OAuth login: Google, GitHub, GitLab providers
- [ ] **T-7.12** — Implement 2FA: TOTP setup, verification, backup codes
- [ ] **T-7.13** — Implement onboarding wizard (first login flow — 5 steps)
- [ ] **T-7.14** — UI: Secrets management page (list, create, edit, versions, diff view, bulk import)
- [ ] **T-7.15** — UI: Registration settings page (admin panel — mode selector, invite management, approval queue)
- [ ] **T-7.16** — UI: Auth pages (forgot password, reset password, accept invite, pending approval, 2FA)

---

## Phase 8 — VPS Providers & Remote Servers (v0.8.0)

- [ ] **T-8.1** — Implement `internal/vps/providers/provider.go` — VPS Provider interface
- [ ] **T-8.2** — Implement `internal/vps/providers/hetzner.go` — Hetzner Cloud full API (create, list, delete, resize, snapshot)
- [ ] **T-8.3** — Implement `internal/vps/providers/digitalocean.go` — DigitalOcean API
- [ ] **T-8.4** — Implement `internal/vps/providers/vultr.go` — Vultr API
- [ ] **T-8.5** — Implement `internal/vps/providers/custom.go` — existing server via SSH (no provisioning API)
- [ ] **T-8.6** — Implement `internal/vps/bootstrap.go` — cloud-init template, SSH bootstrap (install Docker + agent)
- [ ] **T-8.7** — Implement `internal/vps/ssh.go` — SSH connection pool using `golang.org/x/crypto/ssh`
- [ ] **T-8.8** — Implement server provisioning flow: provider API → wait boot → SSH bootstrap → join Swarm → register
- [ ] **T-8.9** — Implement VPS API endpoints per SPECIFICATION.md §18.2
- [ ] **T-8.10** — UI: VPS Providers page (connect, provision, connect existing, cost overview)
- [ ] **T-8.11** — UI: Server provision wizard (provider → region → size → name → create)

---

## Phase 9 — DNS & Topology (v0.9.0)

- [ ] **T-9.1** — Implement `internal/dns/providers/cloudflare.go` — Cloudflare API v4 (create/update/delete records, proxy toggle)
- [ ] **T-9.2** — Implement `internal/dns/sync.go` — DNS sync queue: domain change → create/update DNS record → verify propagation
- [ ] **T-9.3** — Implement auto-subdomain: app created → auto DNS A record
- [ ] **T-9.4** — Implement wildcard DNS setup for auto-subdomains
- [ ] **T-9.5** — UI: Topology View (React Flow):
...
- [ ] **T-10.1** — Implement `internal/marketplace/loader.go` — parse template YAML manifests (embedded + external)
- [ ] **T-10.2** — Implement `internal/marketplace/wizard.go` — JSON Schema → config form data
- [ ] **T-10.3** — Implement `internal/marketplace/deployer.go` — template config → compose YAML → deploy stack
- [ ] **T-10.4** — Implement `internal/marketplace/search.go` — full-text search index over template names, descriptions, tags
- [ ] **T-10.5** — Implement `internal/marketplace/registry.go` — community template sync from GitHub
- [ ] **T-10.6** — Create 20+ initial marketplace templates: WordPress, Ghost, Strapi, Plausible, Umami, MinIO, Gitea, n8n, Uptime Kuma, Vaultwarden, Nextcloud, Metabase, PostgreSQL, MySQL, Redis, MongoDB, Ollama, Open WebUI, Code-Server, Meilisearch
- [ ] **T-10.7** — Implement Marketplace API endpoints per SPECIFICATION.md §18.2
- [ ] **T-10.8** — UI: Marketplace page (category grid, search, filter, template detail, one-click deploy wizard)

---

## Phase 11 — Team Management & RBAC (v0.11.0)

- [ ] **T-11.1** — Implement Team Management API: invite, bulk invite, remove member, change role, activity log
- [ ] **T-11.2** — Implement custom role CRUD: create role with granular permissions, assign to members
- [ ] **T-11.3** — Implement project-level member overrides: per-project role assignment
- [ ] **T-11.4** — Implement team activity feed: who deployed what, when, from which commit
- [ ] **T-11.5** — Implement session management: active sessions list, force logout
- [ ] **T-11.6** — Implement 2FA enforcement toggle (admin forces all members to enable 2FA)
- [ ] **T-11.7** — UI: Team Admin Panel (/team/*) — full panel with members, roles, projects, billing
- [ ] **T-11.8** — UI: Team Members page (list, invite, role change, remove, activity, 2FA status)
- [ ] **T-11.9** — UI: Custom Roles page (create with permission checkboxes, assign)
- [ ] **T-11.10** — UI: Panel switcher (top-left dropdown: Super Admin / Team / Customer)

---

## Phase 12 — Multi-Node & Swarm (v0.12.0)

- [ ] **T-12.1** — Implement `internal/swarm/manager.go` — Docker Swarm init, join token generation
- [ ] **T-12.2** — Implement `internal/swarm/agent.go` — agent mode (`--agent` flag), metrics reporting
- [ ] **T-12.3** — Implement `internal/swarm/placement.go` — node labels, placement constraints
- [ ] **T-12.4** — Implement `internal/swarm/network.go` — overlay network management
- [ ] **T-12.5** — Implement multi-node deploy: deploy as Swarm service with replicas
- [ ] **T-12.6** — UI: Cluster topology map (visual node layout with container placement)

---

## Phase 13 — Billing & Pay-Per-Usage (v0.13.0)

- [ ] **T-13.1** — Implement `internal/billing/plans.go` — plan definitions, limit enforcement
- [ ] **T-13.2** — Implement `internal/billing/metering.go` — 60-second Docker stats collection → hourly rollup
- [ ] **T-13.3** — Implement `internal/billing/quotas.go` — Docker cgroups enforcement, soft/hard limits
- [ ] **T-13.4** — Implement `internal/billing/invoicing.go` — monthly invoice generation with line items
- [ ] **T-13.5** — Implement `internal/billing/stripe.go` — Stripe subscriptions, metered billing, customer portal, webhooks
- [ ] **T-13.6** — Implement billing API endpoints per SPECIFICATION.md §18.2
- [ ] **T-13.7** — UI: Customer billing page (plan, usage bars, invoices, upgrade/downgrade, payment methods)
- [ ] **T-13.8** — UI: Admin billing settings (plans CRUD, pricing, Stripe config, revenue dashboard)
- [ ] **T-13.9** — UI: Public pricing page

---

## Phase 14 — Enterprise & White-Label (v0.14.0)

- [ ] **T-14.1** — Implement `internal/enterprise/whitelabel.go` — branding engine (logo, colors, domain, emails, copyright)
- [ ] **T-14.2** — Implement white-label in React: dynamic logo, colors from API, custom title, hide "Powered by"
- [ ] **T-14.3** — Implement `internal/enterprise/reseller.go` — reseller CRUD, wholesale pricing, customer isolation
- [ ] **T-14.4** — Implement `internal/enterprise/provisioning.go` — Enterprise tenant provisioning API
- [ ] **T-14.5** — Implement `internal/enterprise/compliance.go` — GDPR tools (data export, right to erasure)
- [ ] **T-14.6** — Implement `internal/enterprise/ha.go` — Litestream SQLite replication config
- [ ] **T-14.7** — Implement `internal/enterprise/license.go` — license key validation
- [ ] **T-14.8** — Implement `internal/enterprise/integrations/whmcs.go` — WHMCS provisioning bridge
- [ ] **T-14.9** — Implement `internal/enterprise/integrations/prometheus.go` — `/metrics` OpenMetrics endpoint
- [ ] **T-14.10** — UI: Admin branding settings page (logo upload, color picker, domain config)
- [ ] **T-14.11** — UI: Reseller management (admin panel)
...
- [ ] **T-15.1.1** — Implement `internal/mcp/server.go` — MCP protocol handler
- [ ] **T-15.1.2** — Implement MCP tools: deploy_app, list_apps, get_app_status, scale_app, view_logs, create_database, add_domain, create_backup, provision_server, marketplace_deploy
- [ ] **T-15.1.3** — Implement MCP resources: monster://apps, monster://servers, monster://metrics, monster://logs

### 15.2 CLI Tool

- [ ] **T-15.2.1** — Implement CLI commands per SPECIFICATION.md §22 using `cobra` or stdlib `flag`
- [ ] **T-15.2.2** — CLI subcommands: `serve`, `deploy`, `apps`, `stack`, `image`, `marketplace`, `git`, `vps`, `db`, `domain`, `backup`, `cluster`, `config`
- [ ] **T-15.2.3** — CLI interactive mode: `deploymonster deploy .` auto-detects project, prompts for config

### 15.3 Audit & Logging

- [ ] **T-15.3.1** — Ensure every state-changing API endpoint has audit logging
- [ ] **T-15.3.2** — Implement audit log API: `GET /api/v1/admin/audit-log` with search, filter, export
- [ ] **T-15.3.3** — UI: Audit log page (admin panel — searchable, filterable table)
...
- [ ] **T-15.5.1** — Run `golangci-lint` — fix all warnings
- [ ] **T-15.5.2** — Achieve >70% test coverage on critical paths (auth, deploy, ingress, billing)
- [ ] **T-15.5.3** — Load test: 100 concurrent requests, p95 < 100ms
- [ ] **T-15.5.4** — Load test: proxy 1000 req/s with < 5ms overhead
- [ ] **T-15.5.5** — Binary size check: < 50 MB
- [ ] **T-15.5.6** — Memory test: idle < 100 MB, 50 containers < 500 MB
- [ ] **T-15.5.7** — Startup time check: cold start < 3 seconds

### 15.6 Release

- [ ] **T-15.6.1** — Create `scripts/install.sh` — curl | bash installer
- [ ] **T-15.6.2** — GitHub Actions CI: test → lint → build → release
