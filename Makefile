...
## db-gate: Writers-under-load gate — 64-worker fan-out vs committed p95 baseline (10% threshold)
db-gate:
	@echo "Running writers-under-load baseline gate (internal/db/testdata/concurrent_writes_baseline.json)..."
	DM_DB_GATE=1 DM_DB_GATE_VERBOSE=1 $(GOTEST) -run TestStore_ConcurrentWrites_BaselineGate -v ./internal/db/

## db-gate-baseline: Overwrite the writers-under-load baseline from the current run (intentional re-capture only)
db-gate-baseline:
	@echo "Capturing new writers-under-load baseline — OVERWRITES internal/db/testdata/concurrent_writes_baseline.json"
	@echo "Commit the result with a message explaining WHY the baseline moved."
	DM_DB_GATE=1 DM_DB_GATE_UPDATE=1 $(GOTEST) -run TestStore_ConcurrentWrites_BaselineGate -v ./internal/db/
