...
| R-009 | 3 residual Dependabot alerts (17 closed in Tier 91) | LOW | upstream | As of 2026-04-11 GitHub reported 20 open alerts (11 HIGH + 9 MODERATE) against `master`. Tier 91 closed 17 of them by bumping dependencies: `go.opentelemetry.io/otel*` 1.42.0 → 1.43.0 (CVE-2026-39882, HTTP OTLP exporter unbounded body), `vite` 8.0.3 → 8.0.8 (GHSA-p9ff-h696-f583 + GHSA-v2wj-q39q-566r + GHSA-4w7w-66w2-5vf9, dev-server path-traversal and middleware bypass), plus pnpm `overrides` forcing transitive `vite@7` → `7.3.2` (same three GHSAs via `vitest@3.2.4` → `@vitest/mocker` → `vite@7`) and transitive `lodash@4.17.23` → `4.18.1` (GHSA-r5fr-rjxr-66jc + GHSA-f23m-r3pf-42rh, prototype pollution and ReDoS, reached via the abandoned `dagre@0.8.5` → `graphlib@2.1.8` chain used by the topology editor). The 3 still-open alerts are upstream-blocked: 2 against `github.com/docker/docker` (duplicates of R-001, daemon-side CVEs, no fixed version listed), and 1 against `go.etcd.io/bbolt` (GHSA-6jwv-w5xf-7j27, no upstream fix published yet — tracked, re-check monthly). Severity dropped from MEDIUM to LOW because the remaining alerts are all `fixed=null` upstream waits, not unpatched dependency drift. |
