...
| H-004    | 68   | `internal/billing`      | Stripe webhook replay   | Stripe webhook replay protection was incomplete; a replayed webhook with the same event ID was re-applied. Added deduplication via KV-keyed idempotency store + 24 h retention. |
...
| R-009 | 2 residual Dependabot alerts after BBolt removal | LOW | upstream | Previous dependency drift work closed stale frontend and OpenTelemetry alerts, and BBolt has now been removed from `go.mod`/`go.sum`. Remaining upstream-blocked alerts are Docker daemon-side CVEs with no fixed version listed. Severity remains LOW because the residual items are upstream waits, not local unpatched dependency drift. |
