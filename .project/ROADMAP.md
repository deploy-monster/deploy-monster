...
1. ~~**Fix README inaccuracies.** "97% test coverage" → actual measured number. "25+ marketplace templates" → "116". "9 MCP tools" → fix after scale_app is shipped. "TanStack React Query" (CLAUDE.md) → "custom `useApi` hook". Remove or qualify the dev warning ("not yet ready for production") once Phase 1-3 are green.~~ **DONE (Tier 87).** README and CLAUDE.md rewritten against measured reality: marketplace template count was measured via the live registry (`LoadBuiltins()` → `Count()`) at **56** across **16 categories** — the "116" estimate in this roadmap entry was based on a naive grep that also matched `Slug:` references inside `Related`/`Featured` lists, so it double-counted. API endpoints: 240 (was 224). Handler functions: 222 (was 115). Go source: ~50K LOC (was 27K). Go tests: ~117K LOC (was 47K). Benchmarks: 46 (was 38). Fuzz targets: 15 (confirmed accurate). Binary size: ~24MB stripped (was 22MB). Coverage claim rewritten from a single number to "85%+ (CI gate)" so it stays honest as the codebase grows. Production-readiness warning replaced with a Phase 1–5 completion status line. CLAUDE.md line 94 fixed: `package.json` has no `@tanstack/react-query` dependency, so the "TanStack React Query 5" line was pure drift — replaced with an accurate description of the custom `useApi` hook family. **Effort: 0.5 day.**
2. ~~**OpenAPI regeneration and drift check.** Today `docs/openapi.yaml` is hand-maintained and almost certainly drifted from the 240 registered routes. Write a tool at `cmd/openapi-gen/` that introspects `internal/api/router.go` and emits a spec. Run it in CI and fail on drift.~~ **DONE (Tier 90).** Built as a **drift checker**, not a full generator — full spec emission requires introspecting handler types, request/response schemas, and auth annotations, which is a 2-3 week job in its own right. The narrower, more valuable question a CI job can answer today is: *does every route in code have a matching entry in the spec, and does every spec entry still correspond to real code?* `cmd/openapi-gen/main.go` parses `internal/api/router.go` with a regex scan of `r.mux.Handle(...)` / `r.mux.HandleFunc(...)` calls (filtered against a `routeMethods` allow-list so `PROPFIND` or similar string matches are ignored), parses `docs/openapi.yaml` via `gopkg.in/yaml.v3`, and diffs the two sets. Excludes `/debug/pprof*` and `/metrics*` from both sides. On first run the gap was **232 routes in code vs 88 in spec** — a 144-route code-only drift. Rather than fail CI on day one, the tool ships with a `-bootstrap` mode that writes `docs/openapi-drift-allowlist.txt` with every known-drifted entry, and CI gates on *new* drift only. The allowlist is ratcheting: any entry in the file that no longer corresponds to real drift (because someone closed the gap by adding the spec entry) **also fails CI**, so the file cannot rot silently — every entry is known-drifted *today*, and the only way to remove a line is to fix the spec. 10 unit tests in `main_test.go` cover `parseRouter` (Handle+HandleFunc, pprof/metrics exclusion, unknown-method filter), `parseSpec` (paths+methods, `parameters` key filter), `loadAllowlist` (comments and missing file), `writeAllowlist` (round-trip, nested dir creation), `filterOut`, `mergeSets`, and `routeSet` key sorting. Wired into CI as `make openapi-check` in the `lint` job; `make openapi-bootstrap` regenerates the allowlist during a spec sync. **Effort: 0.75 day.**
3. ~~**ADRs for missing decisions.** Write `docs/adr/0008-encryption-key-strategy.md`, `0009-store-interface-composition.md`, `0010-in-process-event-bus.md`. Each documents why the choice was made, alternatives considered, and under what conditions the decision should be revisited.~~ **DONE (Tier 88).** Two new records landed: `0008-encryption-key-strategy.md` documents the Argon2id + AES-256-GCM vault, the `server.secret_key` → `secrets.encryption_key` fallback, the BBolt-persisted random salt, legacy-salt migration, and `RotateEncryptionKey`; `0009-store-interface-composition.md` documents the 12-sub-interface composition of `core.Store`, the compile-time `var _ core.Store = ...` assertions in both SQLite and Postgres backends, the shared `runStoreContract()` integration harness, and the import-discipline check that keeps `internal/db` invisible to every caller. The roadmap-planned `0010-in-process-event-bus.md` was **not written** because ADR 0006 already covers the in-process event bus completely — writing a second ADR on the same topic would duplicate, not document. `docs/adr/README.md` index updated to list 0008 and 0009 as Accepted. **Effort: 1 day.**
4. ~~**Upgrade guide.** `docs/upgrade-guide.md` exists (untracked). Complete it with a per-version matrix and a migration checklist.~~ **DONE (Tier 89).** Added a per-version compatibility matrix covering every shipped tag from v0.1.0 through HEAD (15 rows, with min-from version, min Go toolchain, config-break flag, DB migrations, agent protocol version) and a three-phase migration checklist (pre-flight, during-upgrade, post-upgrade) derived from operator-facing incident history. Unreleased-version notes rewritten to describe the Phase 1–5 completion state: 88 hardening tiers, Prometheus runtime-metric block on `/metrics/api`, soak-test harness, loadtest regression gate, Go 1.26.2 toolchain bump, `0002_add_indexes` migration, and legacy-vault-salt migration path for pre-Phase-2 installs. **Effort: 1 day.**
5. ~~**Security audit document.** `docs/security-audit.md` exists (untracked). Complete it with the Phase 1-3 fixes captured as "resolved" findings and an honest list of remaining residual risk.~~ **DONE (Tier 89, bundled with item 4).** Added a "Phase 1–5 hardening fixes (closed)" table with 18 rows mapping tier number → subsystem → class → fix summary, covering the lifecycle, context-cancellation, race, replay, and DoS vectors that static analyzers cannot catch but that 88 tiers of hardening closed. Added a "Residual risk register" with 9 rows ranked by severity: R-001 Docker SDK CVEs (upstream), R-002 single-master-key blast radius, R-003 no per-tenant key isolation, R-004 no ciphertext version marker, R-005 no previous-key grace window on vault rotation, R-006/R-007 deferred lint churn, R-008 plaintext audit log, R-009 the 20 Dependabot alerts currently open on the default branch (flagged as a Phase 7 blocker). Every MEDIUM-or-above residual is a documented blocker for a v2.0 tag. **Effort: 1 day.**
6. ~~**`CLAUDE.md` refresh.** Fix the TanStack drift; accurately describe the `useApi`/`useMutation`/`usePaginatedApi` pattern; remove the `vendor-query` chunk name from any mention.~~ **DONE (Tier 87, bundled with item 1 above).** **Effort: 0.25 day.**
...
0. ~~**Address the 20 open Dependabot alerts (R-009).** Blocker for a v1.0 tag per the security audit's residual-risk register. Bump Go module deps with `go get -u`, bump the npm dep tree via pnpm overrides for transitive chains that can't be reached through direct `^` bumps, rerun `govulncheck`, and reconcile the residual list.~~ **DONE (Tier 91).** 17 of the 20 alerts closed: `go.opentelemetry.io/otel*` bumped 1.42.0 → 1.43.0 (CVE-2026-39882 OTLP HTTP exporter unbounded response body); `vite` direct devDep bumped 8.0.3 → 8.0.8 (GHSA-p9ff-h696-f583 + GHSA-v2wj-q39q-566r + GHSA-4w7w-66w2-5vf9); `pnpm.overrides` added to `web/package.json` forcing transitive `vite@7` → `^7.3.2` (same three GHSAs reached via `vitest@3.2.4` → `@vitest/mocker` → `vite@7`) and transitive `lodash@4` → `^4.18.0` (GHSA-r5fr-rjxr-66jc prototype pollution + GHSA-f23m-r3pf-42rh ReDoS, reached via abandoned `dagre@0.8.5` → `graphlib@2.1.8` chain used by the topology editor). 3 alerts remain, all upstream-blocked with `fixed=null`: 2 on `github.com/docker/docker` (daemon-side CVEs, duplicates of R-001) and 1 on `go.etcd.io/bbolt` (GHSA-6jwv-w5xf-7j27, no upstream fix yet — tracked, re-check monthly). R-009 severity downgraded MEDIUM → LOW in the residual-risk register; remaining entries are upstream waits, not unpatched drift. Two pre-existing TypeScript errors surfaced during the `pnpm build` verification were also fixed (`useApi.ts` type narrowing for `api.delete` — it takes `(path, opts)` not `(path, body, opts)` — and a test in `topologyStore.test.ts` that passed an edge type `'connection'` which is not in the `TopologyEdgeType` union). 341 vitest tests + full `go test -short` suite green after the bumps. **Effort: 0.5 day.**

0b. ~~**Close the 11 residual Dependabot alerts that remained open after Tier 91 by removing the stale `web/package-lock.json` and switching `scripts/build.sh` to pnpm.**~~ **DONE (Tier 96).** After Tier 95 pushed, `git push` warned that GitHub still reported 12 Dependabot vulnerabilities on `master` (6 HIGH + 6 MODERATE), which contradicted Tier 91's claim that only 3 were left. Queried the Dependabot alerts API directly and confirmed 11 open alerts — every single one was scoped to `web/package-lock.json`, not `web/pnpm-lock.yaml`: 2× `lodash` (GHSA-r5fr-rjxr-66jc HIGH, GHSA-f23m-r3pf-42rh MEDIUM), 6× `vite` (two copies each of GHSA-p9ff-h696-f583 HIGH, GHSA-v2wj-q39q-566r HIGH, GHSA-4w7w-66w2-5vf9 MEDIUM — three in the `7.x` range and three in the `8.x` range), 1× `bbolt` (upstream-blocked), and 2× `docker/docker` (upstream-blocked). Root cause: `web/package-lock.json` is a stale 198 KB lockfile from a pre-pnpm era (last touched 2026-03-29 in `d672d3e feat: zero-downtime deployment`), still tracked in git alongside the authoritative 123 KB `web/pnpm-lock.yaml`. The Tier 91 `pnpm.overrides` in `web/package.json` correctly pinned transitive `vite@7 → ^7.3.2` and `lodash@4 → ^4.18.0` in `pnpm-lock.yaml`, but pnpm's override semantics never touched the competing npm lockfile, and Dependabot kept dutifully scanning the stale npm lockfile every time the working tree was pushed. The reason the stale lockfile stayed alive: `scripts/build.sh` was hardcoded to `npm ci --silent 2>/dev/null || npm install --silent` + `npm run build`, and `scripts/ci-local.sh` was similarly using `npm install` + `npm test` — both predating the pnpm switch documented in `CLAUDE.md`, `.github/workflows/ci.yml` (which uses `pnpm install --frozen-lockfile`), and the `pnpm.overrides` block itself. Every `make build` therefore regenerated the stale `package-lock.json` against the pre-override version ranges. Fix: (1) `scripts/build.sh` rewritten to require `pnpm` on PATH and invoke `pnpm install --frozen-lockfile` + `pnpm run build`, failing loudly if pnpm is missing with a pointer to `CLAUDE.md`; (2) `scripts/ci-local.sh` similarly migrated from `npm install` + `npm test` to `pnpm install --frozen-lockfile` + `pnpm test`; (3) `web/package-lock.json` deleted via `git rm`; (4) `.gitignore` updated to ignore `web/package-lock.json` so a stray `npm install` can't silently reintroduce the stale file and reopen already-closed alerts. Verification: ran the full `bash scripts/build.sh` pipeline end-to-end — `pnpm install --frozen-lockfile --silent` resolved all deps, `pnpm run build` emitted the usual Vite chunk manifest in 404 ms, `internal/api/static/` was re-embedded (1.1 MB), and the final Go binary built cleanly at `bin/deploymonster` (22 MB). No stray `web/package-lock.json` regenerated. After the next `master` push, Dependabot will rescan and close all 9 `vite`/`lodash` alerts against the now-deleted `web/package-lock.json` manifest; the 3 upstream-blocked alerts (2× `docker/docker`, 1× `bbolt`) stay as documented in R-009. **Meta-lesson for future audits:** when Dependabot reports alert counts that disagree with the working-tree scanner (govulncheck / `pnpm outdated`), check the `manifest_path` of each alert — it's almost always a lockfile that isn't the one your package manager is actually using. **Effort: 0.5 day.**

0a. ~~**Reconcile the stdlib CVE claim against reality via `govulncheck`.**~~ **DONE (Tier 95).** Ran `govulncheck ./...` against HEAD after the Tier 91 cleanup to verify that the Dependabot/toolchain state in the docs actually matched reality. It did not. `govulncheck` surfaced **two active stdlib CVEs** that `docs/security-audit.md` and `docs/upgrade-guide.md` both claimed were already fixed by "bumping Go to 1.26.2": `GO-2026-4870` (unauthenticated TLS 1.3 `KeyUpdate` record → persistent connection retention + DoS in `crypto/tls`) and `GO-2026-4866` (case-sensitive `excludedSubtrees` name constraints → auth bypass in `crypto/x509`). Root cause: the docs described raising the `go` line in `go.mod` from `1.26.1` to `1.26.2`, but that commit never actually landed — `go.mod` still said `go 1.26.1` with no toolchain directive, and `go version` on the build host reported `go1.26.1 windows/amd64`. Fix: added `toolchain go1.26.2` directive to `go.mod` (deliberately **keeping** the `go 1.26.1` minimum-language-version line so downstream module consumers aren't forced to bump their own floor). With `GOTOOLCHAIN=auto` (the default), the next `go build` auto-downloaded 1.26.2 via `dl.google.com/go/go1.26.2.windows-amd64.zip` and compiled against the patched stdlib. Post-fix `govulncheck ./...` output: only the two known upstream-blocked `github.com/docker/docker` findings (`GO-2026-4883`, `GO-2026-4887`) remain, both already allow-listed in the security audit as daemon-side issues not reachable through our client-side usage. `go build ./...` and `go test -short ./...` both green on 1.26.2. `docs/security-audit.md` "Fix applied" section rewritten to describe the actual toolchain directive (not the phantom `go` line bump), with a note that Tier 95 reconciled the documentation against reality. `docs/upgrade-guide.md` already matched and needed no changes. CI jobs continue to pin `go-version: '1.26'` via `setup-go`, which resolves to the latest 1.26.x on each runner — the toolchain directive in `go.mod` now acts as a hard floor so any runner that somehow landed on 1.26.1 still pulls 1.26.2 before compiling. This is exactly the kind of documentation/reality drift that the Phase 1–6 audit was designed to catch, except this one slipped past the docs reviewer and was only caught by a scanner run against the actual build artifact — a useful lesson for future audits: **trust `govulncheck`, not the changelog entry claiming a bump**. **Effort: 0.25 day.**

1. ~~**Version bump.** Adopt the version scheme visible in `STATUS.md` (`v1.4.0` is the current claim; realistic `v0.9.0-rc.1` is probably more honest given Phase 1-6 findings).~~ **DONE (Tier 92).** Landed as `v1.6.0-rc.1`, not `v0.9.0-rc.1`, because monotonicity rules out going backwards — tags `v1.0.0` through `v1.5.0` are already cut. The roadmap's concern was honesty ("don't claim v1.4.0 when the product is really beta-quality"), and the answer to that concern is *the product is no longer beta-quality*: Phase 1–5 hardening is done (88 tiers landed), Phase 6 docs are 100% complete, Phase 7 item 0 (17 of 20 Dependabot alerts closed) landed in Tier 91. The honest next tag is therefore a release *candidate* for the next minor — `v1.6.0-rc.1` — not a pretend-reset to v0.9. Orphan `VERSION` file updated from `0.5.0-alpha` → `v1.6.0-rc.1`. `·project/STATUS.md` rewritten against measured HEAD (version, date, LOC, endpoints, handlers, coverage, fuzz, bench, binary size, and a breakdown of what the rc.1 ships vs what remains in Phase 7.2–7.7). `docs/upgrade-guide.md` per-version matrix: phantom `v1.6.0` row (predicted but never tagged) replaced with the real `v1.6.0-rc.1` row (released 2026-04-11, min-from `v1.0.0`, Go `1.26.2`, no config break, `0002_add_indexes` migration, agent protocol v1). `.github/ISSUE_TEMPLATE/bug_report.md` placeholder bumped from `v1.4.0` / Go 1.26 / Ubuntu 22.04 / Docker 24 → `v1.6.0-rc.1` / Go 1.26.2 / Ubuntu 24.04 / Docker 29. The actual `git tag v1.6.0-rc.1` is a release action and is deliberately **not** performed in this tier — the operator cuts it after Phase 7.2 (`goreleaser --snapshot`) validates. **Effort: 0.25 day.**
2. ~~**Run `goreleaser release --snapshot --clean`** against HEAD and verify every target platform builds, every SBOM generates, every checksum resolves.~~ **DONE (Tier 93).** `goreleaser v2.15.2` installed via `go install github.com/goreleaser/goreleaser/v2@latest`. `goreleaser check` surfaced two v2 schema deprecations in `.goreleaser.yml` — `archives.format: tar.gz` and `archives.format_overrides[*].format: zip` — which were migrated to the v2 syntax (`formats: [tar.gz]` and `formats: [zip]`). A third notice about `dockers` being phased out in favor of `dockers_v2` is forward-looking and left untouched until upstream promotes the new schema to stable. `goreleaser release --snapshot --clean --skip=sbom,docker,before` then ran to completion in **46 seconds** and produced 6 platform archives (`linux/amd64`, `linux/arm64`, `darwin/amd64`, `darwin/arm64`, `windows/amd64`, `windows/arm64`), a `checksums.txt` with 6 SHA-256 entries, `artifacts.json`, `config.yaml`, and `metadata.json`. The `linux/amd64` binary verified as `ELF 64-bit LSB, x86-64, statically linked, stripped`; the `windows/amd64` binary as `deploymonster.exe` inside a proper zip archive; each archive contains the binary plus `LICENSE`, `README.md`, `CHANGELOG.md`, and `monster.example.yaml`. Snapshot binary `deploymonster --version` returns the expected version metadata block with commit SHA and UTC build date. `--skip=sbom` because `syft` is not installed on the build host (SBOM generation is wired in the CI Release job, not the local snapshot path). `--skip=docker` to avoid publishing to GHCR from a snapshot. `--skip=before` to avoid running `go test ./... -count=1` twice since the full test suite was green immediately before the snapshot run. **Important finding for Phase 7.1 operators:** `goreleaser` named the snapshot `0.5.2-SNAPSHOT-92634bb`, not `1.5.0-SNAPSHOT-*`, because the `v1.0.0` through `v1.5.0` tags in the repo are **topologically orphaned** — they were all cut on 2026-03-25 pointing to very early aspirational commits, but `v0.5.2` was cut later on 2026-03-26 from what became the actual trunk. `git rev-list v0.5.2..v1.5.0` returns 0 commits, while `git rev-list v1.5.0..HEAD` returns 186. The topologically-nearest ancestor tag to HEAD is `v0.5.2`, not `v1.5.0`. This does not block the Tier 92 version claim — when the operator cuts `git tag v1.6.0-rc.1` from HEAD, the new tag becomes the nearest ancestor and goreleaser's next run will produce `1.6.0-rc.1` artifacts — but it *does* mean the orphan `v1.x` tags should be left in place (destructive to delete) while the new `v1.6.0-rc.1` tag lives on the real ancestry line. **Effort: 0.5 day.**
...
4. ~~**Release notes.** CHANGELOG.md from current `STATUS.md` + Phase 1-6 delta. Group by "breaking", "security", "feature", "fix".~~ **DONE (Tier 94).** Rewrote `CHANGELOG.md` to lead with a `[1.6.0-rc.1] — 2026-04-11 — Phase 1–7 stabilization release candidate` section grouped by **Breaking** (none — additive only), **Security**, **Features**, **Fixes**, **Performance**, **Documentation**, and **Release engineering**. The Security block covers all 17 closed Dependabot alerts with GHSA identifiers, the Go toolchain bump, the 88 hardening tiers with tier-level citations, and the Argon2id + AES-256-GCM vault with legacy migration path. The Features block covers the OpenAPI drift checker, soak-test harness, loadtest regression gate, runtime metrics, `0002_add_indexes` migration, Postgres store contract parity, and compile-time Store interface assertions. The Release engineering block documents the goreleaser v2 schema migration, the snapshot validation result, and the topologically-orphaned `v1.x` tag footgun for the operator cutting the release. Fixed two drift artifacts in the pre-existing historical sections: (1) the duplicate `[0.1.0]` section that was mistakenly captioned with v1.4.0 content was removed, (2) the `[1.5.0]` section's "116 marketplace templates" claim was corrected to "56 templates across 16 categories" with an explanation that 116 was a naive-grep double-count, and the visual topology editor content that had been incorrectly filed under a phantom `[1.6.0]` 2026-03-29 section was moved into the `[1.5.0]` section where it actually landed. Section order is now strictly reverse-chronological: 1.6.0-rc.1 → 1.5.0 → 1.4.0 → 1.3.0 → 0.1.0. **Effort: 0.5 day.**
