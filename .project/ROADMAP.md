...
- [x] **Version reconciliation** — *closed Sprint 3 truth-up (2026-04-16).* `VERSION` reads `v0.1.7`, git tags exist through `v0.1.7` (including the previously-missing v0.1.3/v0.1.4/v0.1.5/v0.1.6 that this item asked to backfill), and `CHANGELOG.md` has entries for each. The decision went with path (a) — advance VERSION and cut tags — per the v0.1.7 release. Stale roadmap claim.
- [x] **Update `PRODUCTION-READY.md`** — *closed Sprint 3 truth-up (2026-04-16).* File reads "Status: CONDITIONAL GO" with an explicit "100/100 framing is retired" paragraph explaining why the earlier claim didn't survive contact with the follow-up audit. Tracks the conditional-go state honestly and defers numeric detail to `.project/PRODUCTIONREADY.md`. Stale roadmap claim.
...
- [x] Remaining high-severity items — *closed Sprint 3 truth-up (2026-04-16).* Per-item audit: (1) **Rate-limiter TOCTOU** — `internal/api/middleware/global_ratelimit.go:141–167` wraps the check-and-increment (`rl.clients[ip]` lookup → `entry.count >= rl.rate` gate → `entry.count++`) in a single `rl.mu.Lock()/Unlock()` span, so there is no time-of-check / time-of-use window. (2) **BBolt metrics race (RACE-004)** / (3) **connection-tracking race (RACE-003)** — both closed by the medium-severity triage (`security-report/medium-findings-triage.md`) with `SECURITY FIX (RACE-00X)` comments on the remediating lines. (4) **JWT refresh rotation edge case** — covered by the Sprint 3 rotation tests in `internal/auth/jwt_rotation_test.go` (AddPreviousKey accepts both access AND refresh tokens signed under the old key during the grace period; purgeExpiredPreviousKeys compacts the mixed-expired list correctly). Stale roadmap claim.
- [x] Enable race detector in a nightly CI job — *closed Sprint 3 truth-up (2026-04-16).* `.github/workflows/race-nightly.yml` runs `go test -race -count=3 ./...` plus the `integration`-tagged suite under race at `17 3 * * *`, scheduled 30 min before `loadtest-nightly` to avoid runner-quota collision. Includes `workflow_dispatch` for on-demand runs. Stale roadmap claim.
...
- [x] **Cross-provider SSH-key wiring** — *closed Sprint 3 (2026-04-16).* All four provider `Create` payloads now carry `opts.SSHKeyID` under the field name each cloud API expects: DigitalOcean `ssh_keys: [<id|fingerprint>]` (accepts either), Hetzner `ssh_keys: [<id|name|fingerprint>]`, Vultr `sshkey_id: [<uuid>]`, Linode `authorized_keys: [<literal pubkey>]`. Field is omitted when `SSHKeyID` is empty so the existing no-key flow (root password + user-data bootstrap) stays untouched. Pinned by `internal/vps/providers/ssh_key_test.go` — 4 providers × {with, without} = 8 tests that capture the POST body via httptest and assert the field is present/absent and carries the expected value. Linode's contract differs: it's a literal OpenSSH public key rather than an opaque ID, documented inline so future callers resolve the ID before calling `Create`.
...
- [x] **Lift `internal/auth/` coverage to 85%** — *closed Sprint 3 (2026-04-16).* 78.7% → **93.1%** (+14.4 pp, target was 85%). New file `internal/auth/jwt_rotation_test.go` fills the rotation-specific gaps the roadmap called out: `AddPreviousKey` (empty no-op + old-token-accepted-after-rotation, asserts both access and refresh tokens survive the cutover), `purgeExpiredPreviousKeys` (compaction branch when a valid key sits between two expired ones + pure-truncate branch when all are expired), `RevokeAccessToken`/`IsAccessTokenRevoked` (round-trip with fake bolt, nil-bolt fail-open contract, expired-token no-op). Also closes the support surface those rotation paths depend on: `NewJWTService` panic on <32-char secret, `ClaimsFromContext`/`ContextWithClaims` round-trip (including empty-context nil-cast), and `HashPassword`/`HashAPIKey` >72-byte bcrypt error branches.
- [x] **Lift `internal/db/` coverage to 85%** — *closed Sprint 3 (2026-04-16).* 78.9% → **88.7%** (+9.8 pp, target was 85%). The big drag was 17 `PostgresDB` methods at 0% (no live-Postgres tests in CI); added `internal/db/postgres_gaps_test.go` using go-sqlmock to cover all 17: `DB()`, `GetAppByName`, `UpdateDeployment`, `ListDeploymentsByStatus`, `AtomicNextDeployVersion`, `hashAppIDToLockID`, `GetDomain`, `DeleteDomainsByApp`, four secret lookup methods, usage records, and three backup methods. The transaction-rollback paths the roadmap called out live in `AtomicNextDeployVersion` (RACE-002 incident) — six subtests exercise every branch including BeginTx error, advisory-lock error, Scan error (both trigger the deferred `tx.Rollback()`), NULL-max first-deploy, and Commit error. Error-path tests are paired with success-path tests so a regression that drops an error return fails the test. All pass.
- [x] **Add integration test: multi-tenant authorization matrix** — *closed Sprint 3 (2026-04-16).* Two complementary tests now guard every app-scoped endpoint against cross-tenant leaks: (a) `FuzzRouter_CrossTenant` + `TestRouter_CrossTenantSeedCorpus` (already in tree, `internal/api/router_fuzz_test.go`) walk **38 resource-scoped GETs** with a foreign-tenant resource ID plus 13 injection probes — oracle rejects any 2xx; (b) `TestRouter_CrossTenantMutationMatrix` (added this sprint, `internal/api/router_cross_tenant_mutation_test.go`) walks **38 mutation endpoints** (PATCH/PUT/POST/DELETE across app lifecycle, all 16 config PUTs, 12 collection POSTs, and the admin-only transfer) and asserts each returns non-2xx against a foreign-tenant app. The test also guards against a misconfiguration where the bearer token fails to reach the handler (asserts !401). Total matrix coverage: **76 route×foreign-resource cells**, all PASS. Prevents the exact regression class that AUTHZ-001/006/007/008 were — a mutation endpoint forgetting the `requireTenantApp` gate.
...
- [x] **Publish SLAs** — *closed Sprint 3 (2026-04-17).* `docs/sla.md` commits DeployMonster to specific numbers on the single-master single-tenant topology: HTTP read-path p95 < 50–100 ms per endpoint + >= 750 req/s aggregate at concurrency=20; mutation-path p95 < 500 ms for app-create, < 200 ms for env-write, < 800 ms for login (bcrypt cost 13 dominates); deploy-pipeline control-plane overhead < 8 s p95 excluding clone / build / pull; concurrency targets of >= 10 in-flight deploys, >= 100 running apps, >= 50 tenants, >= 20 agents per master; availability 99.5% rolling 30 d with < 30 s MTTR. Targets are baseline-relative: the regression gate (`make loadtest-check`) already fails on >= 10 % drift vs the committed baseline; the doc adds absolute floors so dev-box seed numbers can't mask a slow-hardware regression. Out-of-scope items (multi-master HA SLA, managed-SaaS SLA, build-throughput SLA, >20-agent fleet) are called out explicitly so the published commitment is unambiguous. Revisit triggers (hardware change, Go toolchain bump, middleware addition, store-interface swap, deliberate perf win) are documented inline so the next re-baseline doesn't need roadmap coordination.
...
- [x] **DB indexes audit** — *closed Sprint 3 (2026-04-17).* Swept every `WHERE`/`ORDER BY` clause in `internal/db/sqlite.go` against the existing schema. 0001_init.sql's PRIMARY KEY + UNIQUE constraints already cover `id = ?`, `email = ?`, `fqdn = ?`, `slug = ?` lookups; 0002_add_indexes.sql adds 11 single-column foreign-key indexes for the list-by-FK pattern (`tenant_id = ?`, `app_id = ?`, etc.). The audit found three gaps on hot-path composite lookups: `secrets(scope, name)` (called on every secret resolution during deploy config loading), `deployments(status)` (called at master boot for RACE-002 reclaim), and `applications(tenant_id, name)` (GetAppByName from REST + CLI). Migration 0003 (`0003_add_hot_query_indexes.{sql,pgsql.sql}` + down counterparts) adds composites for all three in both dialects. `TestMigrations_RollbackPartial` rewritten to walk the expected-index set symbolically (Rollback(1) peels 0003-only, Rollback(2) peels 0002 too) rather than pinning to a migration number, so the next audit that adds an 0004 doesn't have to touch the test.
- [x] **Bundle-size budget** — *closed Sprint 3 (2026-04-16).* `web/scripts/check-bundle-size.mjs` gzips the main entry chunk (`dist/assets/index-*.js`) and fails with exit 1 if > 300 KB. Wired as `pnpm run check:bundle` and runs in the `test-react` CI job after `pnpm run build`. Current baseline: 7.93 KB gzipped (budget is 300 KB — the check is a catastrophic-regression guard, not an optimization floor). Intentionally scoped to the main chunk only: lazy-loaded route chunks (Topology, AppDetail, etc.) and vendor splits (vendor-react, vendor-graph) are the product of deliberate code-splitting and shouldn't be measured by the same bar.
- [x] **Lazy-load topology editor vendor bundle** — *closed Sprint 3 truth-up (2026-04-16).* Already lazy: `web/src/App.tsx:32` uses `lazy(() => import('./pages/Topology'))`, and `web/vite.config.ts:32–34` `manualChunks` routes `@xyflow/react` + `dagre` into a dedicated `vendor-graph` chunk. Build output confirms: `Topology-*.js` 51.54 KB (gzip 11.15 KB) and `vendor-graph-*.js` 175.15 KB (gzip 57.79 KB) are both separate from the 28 KB main chunk. The claim that these were "heavy and in the main chunk" was stale. Stale roadmap claim.
- [x] **BBolt batching** — *closed Sprint 3 truth-up (2026-04-16).* Audited every `bolt.Set(` call site (41 files). Hot paths (`middleware/ratelimit.go`, `middleware/tenant_ratelimit.go`, `middleware/idempotency.go`, `webhooks/delivery_log.go`) each do exactly one `Set` per incoming request, not N-per-request. Config-writer handlers (`redirects.go`, `response_headers.go`, `sticky_sessions.go`, etc.) marshal the entire list into a single JSON blob before one `bolt.Set` — that's already the batched pattern. The multi-version rotation path lives on SQL (`secrets/module.go:migrateLegacyVault` loops `UpdateSecretVersionValue`), not bolt. `BatchSet` exists on `BoltStore` (`bolt.go:112`) and is used by the `db_gap_coverage_test.go` regression suite, but there are zero production call sites with N>1 `Set` in a loop that would benefit from migrating to it. The roadmap's claim ("hot paths call Set in a loop") was speculative — the actual call graph doesn't match the concern. Leaving `BatchSet` in place for future callers that genuinely need it.
...
- [x] **Rewrite README** — *closed Sprint 3 (2026-04-17).* Every number in the README was re-measured against HEAD: fuzz targets 15 → **17**, benchmarks 46 → **44**, "240 REST endpoints" → **205** router-registered (with explicit "90 documented in OpenAPI, remaining 115 in the drift allowlist" framing so the gap is visible rather than aspirational). `npm install` → `pnpm install` fixed (same bug that was in CONTRIBUTING.md). Duplicate badge block at top removed. Dropped the "Zero Dependencies" copy — unprovable given Docker + SQLite + OS runtime deps. Coverage claim replaced with truthful "package-weighted 85.9 %" (filtered to exclude `tests/loadtest` + `tests/soak` harnesses whose `*_test.go` files are 1-line smoke tests that exist only to keep the binaries compiling under `go test`; raw is 84.2 %) + per-hot-package breakdown (auth 93 %, db 89 %, marketplace 96 %). The CI gate now filters the same packages from `coverage.out` before the threshold check — closes the Test-job-red state that had flagged as the third CI failure alongside vitest and OpenAPI drift. Added a full **Known Limitations** section naming the 8 things we explicitly don't pretend to support: multi-master HA, Kubernetes target, AWS EC2, Route53, partial OpenAPI coverage, continue-on-error E2E, stubbed OpenTelemetry, no plugin system. Landing-page pointer block rewritten to link the ADRs, the runbook, the upgrade guide, and CONTRIBUTING.md so the first-click experience is "here's where to go" not "here are promises."
- [x] **Regenerate `.project/SPECIFICATION.md` scope framing** — *closed Sprint 3 (2026-04-17).* Full rewrite would be 2 days and destroy the existing 4596-line design-intent document that's genuinely useful for "what is this project supposed to look like." Instead: added a **"Document status (read this first)"** banner at the top that makes the authority stratification explicit — spec is design-intent, README is binary-truth (with Known Limitations), ROADMAP is sprint-tracked work-status, ADRs are the load-bearing "why" records. The banner calls out by name the items where spec says "planned" but the roadmap has explicit status (Route53, AWS EC2, marketplace 91-not-150+), with the rule that when spec and tree disagree the tree wins and the spec gets a follow-up edit. Pre-existing Route53 / AWS EC2 entries inside the doc already carry *(Planned)* / *(Beyond 1.0 — deferred)* annotations (lines 130, 150, 521, 875, 1292, 1295) so the ongoing drift signal is minimal. README already has a direct pointer to both the spec and the roadmap in its landing-page doc table.
- [x] **Rewrite `.project/TASKS.md`** — *closed Sprint 3 (2026-04-17): retired.* All 251 tasks across 15 phases are complete (the initial-build checklist did its job). File is preserved as an immutable historical record. New header block declares "Status: RETIRED — historical record" and routes current-work tracking to `ROADMAP.md`. The original companion-to-SPECIFICATION-and-IMPLEMENTATION framing moved to a "historical" qualifier so a future reader immediately understands this is a frozen artifact, not an active plan. Rewriting from scratch would be pure make-work — the only concrete harm was the drift signal ("this looks like the current plan"); the retirement note is the minimum intervention that closes that signal without destroying the build log.
...
- [x] **Contributor guide** — *closed Sprint 3 (2026-04-17).* `CONTRIBUTING.md` rewritten against the actual Makefile and toolchain. Fixed: `npm install` → `pnpm install` (the old doc would create a broken lockfile); added every relevant make target (`test-short`, `test-integration`, `test-integration-postgres`, `db-gate`, `loadtest-check`, `soak-test`, `coverage`, `bench`, `lint`, `vet`, `fmt`); documented the `pgintegration` build tag + exact Postgres `docker run` one-liner with `TEST_POSTGRES_DSN`; added fuzz-target invocation; added frontend test + bundle-budget commands; cross-referenced the 10 ADRs, `.project/` tree, and `CLAUDE.md`. Security-disclosure path pointed at `security@ecostack.ee`. Commit-message section documents the `sprint(N):` scope and the Claude `Co-Authored-By` trailer convention used throughout the session history.
- [x] **Deploy runbook** — *closed Sprint 3 (2026-04-17).* `docs/runbook.md` is the single-entry-point operator runbook. Scenario table maps 11 P0/P1/scheduled events to the authoritative doc: upgrade → `upgrade-guide.md`, JWT rotation → `secret-rotation.md`, socket hardening → `docker-socket-hardening.md`, agent upgrades → upgrade-guide's agent section. Two scenarios the existing docs didn't spell out end-to-end are covered inline: (a) **DB migration failure** mid-boot — step-by-step recovery that explicitly rules out the hand-apply-migration anti-pattern and routes to restore-from-backup; (b) **Disaster recovery** — fresh-host restore from S3 backup with the critical prerequisite (master secret must live outside DeployMonster or encrypted data is unrecoverable), plus the agent re-enrolment step that people forget. Closes with a post-incident checklist that feeds observations back into this runbook + the roadmap.
...
- [x] **SBOM audit** — *closed Sprint 3 truth-up (2026-04-17).* Both halves already wired: `.goreleaser.yml:43–46` emits SPDX-JSON SBOMs per-archive via `anchore/sbom-action/download-syft@v0` (syft v1.18.1 pinned in `.github/workflows/release.yml:42–44`) — every tag push produces `deploymonster_vX.Y.Z_<os>_<arch>.sbom.json` attached to the GitHub Release. Trivy scan of the freshly pushed GHCR image runs in the same workflow (lines 67–75) with `exit-code: '1'`, `severity: HIGH,CRITICAL`, `ignore-unfixed: true`, pinned to the post-supply-chain-compromise safe SHA. HIGH/CRITICAL fixable CVEs fail the release; unfixable upstream CVEs are tolerated so a stuck dep doesn't block a tag. "Subscribe to CVE feeds for direct deps" is the one remaining bullet and is inherently an operator-level subscription (OSV.dev / GHSA watch lists) rather than a code change — recorded here for ops onboarding but not a tracked engineering item.
- [x] **Release notes** — *closed Sprint 3 truth-up (2026-04-17).* When the roadmap was written, v0.1.2 was the last tagged version. Since then CHANGELOG.md has gained entries for every intermediate tag (0.1.3 installer + static-asset CORS bypass, 0.1.4 CORS rewrite (noted as later reverted in 0.1.7), 0.1.5 duplicate tag, 0.1.6 comprehensive UX overhaul, 0.1.7 Sprint 1 hardening) all using the Keep-a-Changelog shape with explicit **Security** / **Features** / **Fixes** / **Performance** sections. Security fixes are called out in their own section per the Phase 7 roadmap ask. Stale roadmap claim.
