...
1. **Master/agent end-to-end.** Spawn a real master process, spawn a real agent process in a separate goroutine with the `--agent` flag, exercise the full protocol (ping/pong/container.*/metrics.collect/health.check). Today this is mocked at the wire level. **Effort: 2-3 days.**
2. **Let's Encrypt staging integration.** Request a cert from LE staging during CI. Gated on an env var so CI doesn't abuse the staging quota. **Effort: 1.5 days.**
...
3. **Lock-free routing table in ingress.** Replace the `sync.RWMutex` on the routing table with an `atomic.Value` holding an immutable snapshot. Writers build a new snapshot and swap; readers never block. Benchmark before/after under `wrk -c 200 -d 30s`. **Effort: 1 day.**
4. **Per-tenant build queue.** Implemented in Phase 3.7; in this phase, add benchmarks and a load-test harness under `tests/loadtest`. **Effort: 0.5 day.**

### 5.3 Load testing
5. **`tests/loadtest` already exists in the Makefile (`make loadtest`).** Run it against a production-shaped config and commit a baseline. Alert on regression ≥ 10%. **Effort: 1.5 days.**
6. **Soak test.** 24-hour run at 10% of load-test peak. Verify no goroutine leak, no memory climb, no unbounded audit log growth. **Effort: 1 day** of engineer time + 24h wall clock.
